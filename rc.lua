...
-- battery percentage
battery_text = awful.widget.watch("cat /sys/class/power_supply/BAT0/capacity", 30, function(widget, stdout)
...
battery_charging = awful.widget.watch("cat /sys/class/power_supply/AC/online", 5, function(widget, stdout)
    for line in stdout:gmatch("[^\r\n]+") do
        if line == "1" then
            widget.text = " +"
        else
...
battery_bar = awful.widget.watch("cat /sys/class/power_supply/BAT0/capacity", 30, function (widget, stdout)
    charge = 0
    for line in stdout:gmatch("[^\r\n]+") do
